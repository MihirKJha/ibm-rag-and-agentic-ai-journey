...
def main():
    ...
    try:
        ...
        print("   Powered by IBM Granite & ChromaDB Vector Search")
        print("=" * 55)
...
def prepare_context_for_llm(query: str, search_results: List[Dict]) -> str:
    ...
    for i, result in enumerate(search_results[:3], 1):
        ...
        food_context.append(f"  - Category: {result['food_category']}")
...
def generate_llm_rag_response(query: str, search_results: List[Dict]) -> str:
    ...
    try:
        ...
        prompt = f'''You are a helpful food recommendation assistant. A user is asking for food 
        recommendations, and I've retrieved relevant options from a food database.
...
def handle_enhanced_rag_query(collection, query: str, conversation_history: List[str]):
    ...
    for i, result in enumerate(search_results[:3], 1):
        ...
        print(f"   📍 {result['cuisine_type']} | 🔥 {result['food_calories_per_serving']} cal | 🔥 {result['food_category']} | 📈 {result['similarity_score']*100:.1f}% match")
        if i < 3:
            print()
...
def generate_llm_comparison(query1: str, query2: str, results1: List[Dict], results2: List[Dict]) -> str:
    ...
    try:
        ...
        comparison_prompt = f'''You are analyzing and comparing two different
        food preference queries. Please provide a thoughtful comparison.
