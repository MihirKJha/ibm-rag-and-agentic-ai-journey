    ...
    try:
        ...
        for node in nodes:
            ...
            if not text or not isinstance(text, str) or text.strip() == "":
                logger.warning(f"Skipping invalid node: {text}")
                continue
            
            clean_nodes.append(node)

        logger.info(f"Final {len(clean_nodes)} nodes after removing invalid ones")    
...
def extract_text_fields(profile_data: str) -> List:
    texts = []
    
    # Headline / summary
    if profile_data.get("summary"):
        texts.append(profile_data["summary"])
