...
def process_profile(linkedin_url, api_key, use_mock, selected_model):
    ...
    try:
        ...
        profile_data = extract_linkedin_profile(
            linkedin_url, api_key if not use_mock else None, mock=use_mock
        )
    ...
    with gr.Blocks(title="LinkedIn Icebreaker Bot") as demo:
        ...
        with gr.Tab("Process LinkedIn Profile"):
            with gr.Row():
                ...
                with gr.Column():
                    result_text = gr.Textbox(value="", label="Initial Facts", lines=10)
                    session_id = gr.Textbox(value="", label="Session ID", visible=False)
